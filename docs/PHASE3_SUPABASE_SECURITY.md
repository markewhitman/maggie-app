...
- The code has backward-compatible fallbacks for older Supabase schemas, so the app still works before the SQL migration is applied, but request scoping will not be strict until the migration is applied.
